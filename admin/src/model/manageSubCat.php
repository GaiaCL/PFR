...
function requestSubCategorieManagement() {
    ...
    $sql = 'SELECT sc.id_categorie, sc.id, sc.name_sub_categorie, sc.description, c.name_categorie as nameCat 
    FROM `sub_categories` as sc INNER JOIN categories as c ON sc.id_categorie = c.id LIMIT :premier, :parpage;';
    $query = dbConnect()->prepare($sql);
