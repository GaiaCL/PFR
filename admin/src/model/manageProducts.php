...
if(isset($_GET['id'])){
 ...
 function requestProductsManagement() {
  ...
  $sql = 'SELECT p.id_sub_categories, p.id, p.name_products, p.description, p.price, p.date_ajout, p.is_enabled, sc.name_sub_categorie as nameSubCat FROM `products` as p INNER JOIN sub_categories as sc ON p.id_sub_categories = sc.id LIMIT :premier, :parpage;';
  $query = dbConnect()->prepare($sql);
